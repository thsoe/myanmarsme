				...
				for ($i=0; $i<count($result); $i++)
				{
					...
					if(! is_null($directorycompany))
					{
						...
						$guest_str .= "<div style='width:100%;height:120px;'><div style='width:50%;float:left;'>
							<img src='" . $company->getLogo() . "' style='margin:10px;' /></div>
							<div style='width:50%;float:right;'>" . $company->getName() . "<br />" . 
							$company->getDescription() . "</div></div>";
					}
					$guest_str .= "<br /><div style='margin:10px;'><strong style='font-size:17px;'>About</strong>";
					/* if($is_private == 2)	
